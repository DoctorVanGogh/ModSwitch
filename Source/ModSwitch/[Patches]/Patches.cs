    ...
    internal class Patches {
            ...
            public class InjectSearchBox {
                public static Rect AllocateAndDrawSearchboxRect(Rect r) {
                    ...
                    ModsConfigUI.Search.DoSearchBlock(
                        new Rect(r.x + ModsConfigUI.Search.buttonsInset, r.y + ModsConfigUI.Search.buttonsInset, r.width - 2 * ModsConfigUI.Search.buttonsInset,
        ...
        public class Page_ModsConfig_DoModRow {
            ...
            public class InjectRightClickMenu {
                ...
                public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator ilGen) {
                    ...
                    var idxCheckboxLabeledSelectable = instr.FirstIndexOf(ci => ci.opcode == OpCodes.Call && ci.operand == ModsConfigUI.miCheckboxLabeledSelectable);
                    if (idxCheckboxLabeledSelectable == -1) {
                        Util.Warning("Could not find anchor for ModRow transpiler - not modifying code");
                        return instr;
                    }
                    var lblBlockEnd = (Label) instr[idxCheckboxLabeledSelectable + 1].operand;
                    var idxBlockEnd = instr.FindIndex(idxCheckboxLabeledSelectable + 1, ci => ci.labels.Contains(lblBlockEnd));
                    if (idxBlockEnd == -1) {
                    ...
                    var localColor = ilGen.DeclareLocal(typeof(Color));
                    var lblExistingClickCode = ilGen.DefineLabel();
                    var lblNoClick = ilGen.DefineLabel();
                    ...
                    instr.InsertRange(idxCheckboxLabeledSelectable - 4, new[] {
                                                                                  new CodeInstruction(OpCodes.Ldarg_2),
                                                                                  new CodeInstruction(
                                                                                      OpCodes.Call, AccessTools.Method(typeof(ModsConfigUI.Helpers), nameof(ModsConfigUI.Helpers.SetGUIColorMod))),
        ...
        public class Page_ModsConfig_PostClose {
            ...
            public static bool Prefix(Page_ModsConfig __instance) {
                ModsConfig.Save();
                if ((int) ModsConfigUI.fiPage_ModsConfig_ActiveModsWhenOpenedHash.GetValue(__instance) != ModLister.InstalledModsListHash(true)) ModsConfigUI.OnModsChanged();
                return false;
            }
        }
        ...
        public class MainMenuDrawer_DoMainMenuControls {
            ...
            private static bool InjectDeferedRestartHint(List<CodeInstruction> instructions, ILGenerator ilGen, string anchor) {
                ...
                var idxAddNewColonyOption = instructions.FindIndex(idxNewColony, ci => ci.opcode == OpCodes.Newobj && ci.operand == ciNewListableOption);
                ...
                var idxNewColonyAction = instructions.FindLastIndex(idxAddNewColonyOption, idxAddNewColonyOption - idxNewColony, ci => ci.opcode == OpCodes.Ldsfld);
